...
public class RideProcessorService {
    ...
    private List<MatchedPoint> getEnrichedPoints(Ride ride, TraceResponse traceResponse) throws IllegalArgumentException {
        ...
        for (int i = 0; i < traceResponse.getMatchedPoints().size(); i++) {
            ...
            Coordinate coordinate = new Coordinate(resultPoint.getLon(), resultPoint.getLat());
            ...
            RidePoint referenceRidePoint = new RidePoint();
            referenceRidePoint.setId(requestPoint.getRidePointId());
            matchedPoint.setRidePoint(referenceRidePoint);
        ...
        for (List<MatchedPoint> edge : sortedPoints) {
            ...
            for (int i = 0; i < edge.size(); i++) {
                ...
                for (int j = 1; i + j < edge.size() && distances.size() < sizeLoopCheck; j++) {
                    Coordinate compareCoordinate = edge.get(i + j).getCoordinate();
    ...
    private void getIntersections(Ride ride, List<List<MatchedPoint>> sortedPoints,
        ...
        for (IntersectionEdge edge : newEdges) {
            ...
            edge.setLine(point.getLine());
            edge.setPrevLine(point.getPrevLine());
            edge.setNextLine(point.getNextLine());
        ...
        for (IntersectionNode node : newNodes) {
            node.setEndLine(node.getMatchedPoints().getLast().getLine());
            node.setStartLine(node.getMatchedPoints().getFirst().getLine());

            addLastPointOfPreviousElement(node);
            applyIntersectionProperties(node, ride);
            setClusterAndStreetNamesOnIntersectionNode(node);
        }
    ...
    private void setClusterAndStreetNamesOnIntersectionNode(IntersectionNode intersectionNode) {
        ...
        TrafficSignalCluster cluster = intersectionNode.getMatchedPoints().get(1).getInIntersectionCluster();
    ...
    private void applyIntersectionProperties(IntersectionBase element, Ride ride) {
        ...
        Coordinate[] lineString = points.stream().map(MatchedPoint::getCoordinate).toArray(Coordinate[]::new);
