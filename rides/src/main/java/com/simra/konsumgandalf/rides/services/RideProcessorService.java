...
public class RideProcessorService {
    ...
    private List<MatchedPoint> getEnrichedPoints(Ride ride, TraceResponse traceResponse) throws IllegalArgumentException {
        ...
        if (traceResponse.getMatchedPoints().isEmpty()) return matchedPoints;

        MatchInformation prev = traceResponse.getPayloadCoordinates().getFirst();
        Point prevRawGPSLocation = geometryFactory.createPoint(
                new Coordinate(prev.getLng(), prev.getLat()));
        for (int i = 0; i < traceResponse.getMatchedPoints().size(); i++) {
            ...
            matchedPoint.setGeom(geometryFactory.createPoint(new Coordinate(resultPoint.getLon(), resultPoint.getLat())));
            ...
            matchedPoint.setRawGPSLocation(geometryFactory.createPoint(
                    new Coordinate(requestPoint.getLng(), requestPoint.getLat())));
    ...
    /**
     * Sorts the points into lists with the same valhalla edge id.
     * Enriches the points with the following information based on the edge it belongs to:
     *  osmLine: osmLine of edge
     *  prevOsmLine: osmLine of previous edge
     *  nextOsmLine: osmLine of next edge
     *  valhallaEdgeId: valhallaEdgeId of edge
     *  prevValhallaEdgeId: valhallaEdgeId of previous edge
     *  nextValhallaEdgeId: valhallaEdgeId of next edge
     *  trafficSignalClusters: intersection of edge with traffic signal clusters
     *      or trafficSignalClusters from previous and next edge if no corresponding edge
     * If there is no edge the line and id information is null
    ...
    private List<List<MatchedPoint>> getSortedPointsAndEnrichPoints(Ride ride, TraceResponse traceResponse) throws IllegalArgumentException {
        List<List<MatchedPoint>> sortedPoints = getSortedPoints(
                getEnrichedPoints(ride, traceResponse), getEnrichedEdges(traceResponse));
     ...
     *  valhallaEdgeId: valhallaEdgeId of edge
     *  trafficSignalClusters: intersection of edge with traffic signal clusters
     * If there is no edge the information is null
    ...
    private List<List<MatchedPoint>> getSortedPoints(List<MatchedPoint> points, List<Edge> edges) {
        ...
        if (points.isEmpty()) return sortedPoints;
        ...
        for (MatchedPoint matchedPoint : points) {
            ...
            if (geoService.distance(currentPoints.getLast().getGeom(),
                    matchedPoint.getGeom()) < 0.1) continue;
    ...
    private void discardUnMatchedPointsAt(List<List<MatchedPoint>> points, boolean front) {
        while (!points.isEmpty() && front ?
                points.getFirst().getFirst().getValhallaEdgeId() == null :
                points.getLast().getLast().getValhallaEdgeId() == null) {
            if (front) points.removeFirst(); else points.removeLast();
        }
    }
        ...
        for (List<MatchedPoint> edge : sortedPoints) {
            ...
            for (int i = 0; i < edge.size(); i++) {
                ...
                if (geoService.distance(currentPoint.getRawGPSLocation(), currentPoint.getPrevRawGPSLocation()) > 60) {
                    ...
                    // With a low threshold of shorter than 100 meters this may cut even objectively fine rides,
                ...
                while (!pointsInRage.isEmpty() && geoService.distance(currentPoint.getGeom(),
                        pointsInRage.getFirst().getGeom()) > 50) {
    ...
    private void removeOnStartEndCondition(List<List<MatchedPoint>> routeParts) {
        ...
        if (!routeParts.isEmpty()) routeParts.removeFirst();
        trimRouteParts(routeParts, true);

        trimRouteParts(routeParts, false);
        if (!routeParts.isEmpty()) routeParts.removeLast();
        trimRouteParts(routeParts, false);
    }
    ...
    private void trimRouteParts(List<List<MatchedPoint>> routeParts, boolean front) {
        while (!routeParts.isEmpty()) {
            ...
            if (!trim) return;
            if (front) routeParts.removeFirst(); else routeParts.removeLast();
        }
    }
    ...
    private boolean checkEndStartCondition(List<MatchedPoint> edge) {
        ...
        for (MatchedPoint currentPoint : edge) {
            ...
            if (distanceFromTracePoint == null) continue;
            ...
            if (distanceFromTracePoint < minDistance) minDistance = distanceFromTracePoint;
        }
    ...
    private <T extends IntersectionBase> IntersectionBase finalizeElement(T element, List<T> list, IntersectionBase previous) {
    ...
    private void fillIntersectionLists(List<List<MatchedPoint>> sortedPoints,
                                       List<IntersectionEdge> edgeList, List<IntersectionNode> nodeList) {
        ...
        for (List<MatchedPoint> sortedEdge : sortedPoints) {
            // Each sorted edge has exactly one osm id, as long as no points are in an intersection
        ...
        if (!nodeList.isEmpty() && sortedPoints.getFirst().getFirst().getInIntersection()) {
            ...
            // The last node is skipped as well if the ride ends with it, as it is not finalized
            nodeList.removeFirst();
        }
    }

    private void getIntersections(Ride ride, List<List<MatchedPoint>> sortedPoints,
                                 List<IntersectionEdge> allEdges, List<IntersectionNode> allNodes) {
        ...
        for (IntersectionEdge edge : newEdges) {
            ...
            addLastPointOfPreviousElement(edge); // This prepends a point with a (usually) different osmId
