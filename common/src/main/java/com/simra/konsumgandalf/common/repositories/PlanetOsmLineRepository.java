...
public interface PlanetOsmLineRepository extends JpaRepository<PlanetOsmLine, Long> {
	...
	@Query(value = """
			...
			""", nativeQuery = true)
	List<Long[]> findClosestStreetSegments(@Param("streetSegmentIds") List<Long> streetSegmentIds,
			@Param("incidentIds") Long[] incidentIds, @Param("lngs") Double[] lngs, @Param("lats") Double[] lats);
